    ...
    if request.method == 'GET':
        ...
        try:
            data = CustomData(
                ...
                reading_score=float(request.form.get('reading score')),  # Corrected
                writing_score=float(request.form.get('writing score'))  # Corrected
            )
            ...
            # Initialize and make prediction
            predict_pipeline = PredictPipeline()
            print("Mid Prediction")
            results = predict_pipeline.predict(pred_df)
            print("After Prediction")

            # Render prediction result in the HTML
